...
const LogsIcon = ({ className = "w-6 h-6" }) => (
...
const UserCircleIcon = ({ className = "w-4 h-4" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
...
const Sidebar = ({ activeItem, setActiveItem }) => {
    const navItems = [
        ...
        { name: 'Logs', icon: <LogsIcon />, color: 'text-gray-500' },
...
const LogsView = () => {
    const logs = [
        { id: 1, dateTime: '2024-03-15 14:32:15', type: 'assignment', description: "Documento 'Contrato_2024.pdf' atribuído ao usuário", user: 'João Silva' },
        { id: 2, dateTime: '2024-03-15 14:28:03', type: 'upload', description: "Upload de 'Proposta_Comercial.pdf' no workflow 'Análise Comercial'", user: 'Maria Santos' },
        { id: 3, dateTime: '2024-03-15 14:25:41', type: 'completion', description: "Documento finalizado no workflow 'Revisão Legal'", user: 'Pedro Costa' },
        { id: 4, dateTime: '2024-03-15 14:20:18', type: 'error', description: "Erro ao processar ferramenta 'Extração de Dados'", user: 'Sistema' },
        { id: 5, dateTime: '2024-03-15 14:15:52', type: 'success', description: "Ferramenta 'Análise de Sentimento' processada com sucesso", user: 'Sistema' },
        { id: 6, dateTime: '2024-03-15 14:12:34', type: 'stage', description: "Documento avançou para etapa 'Aprovação Final'", user: 'Ana Lima' },
        { id: 7, dateTime: '2024-03-15 14:08:21', type: 'upload', description: "Upload de 'Relatório_Q1.pdf' no workflow 'Análise Financeira'", user: 'Carlos Mendes' },
        { id: 8, dateTime: '2024-03-15 14:05:09', type: 'warning', description: "Motor de IA 'GPT-4' temporariamente indisponível", user: 'Sistema' },
        { id: 9, dateTime: '2024-03-15 14:01:45', type: 'assignment', description: "Documento 'Orçamento_2024.xlsx' atribuído ao usuário", user: 'Laura Oliveira' },
    ];

    const getTypeStyles = (type: string) => {
        switch (type) {
            case 'assignment': return { bg: 'bg-brand-accent', icon: <UserCircleIcon className="w-3 h-3 text-white" />, label: 'assignment' };
            case 'upload': return { bg: 'bg-brand-accent', icon: <UploadIcon className="w-3 h-3 text-white" />, label: 'upload' };
            case 'completion': return { bg: 'bg-green-100', text: 'text-green-800', icon: <CheckIcon className="w-3 h-3" />, label: 'completion' };
            case 'error': return { bg: 'bg-red-500', icon: <XIcon className="w-3 h-3 text-white" />, label: 'error' };
            case 'success': return { bg: 'bg-green-100', text: 'text-green-800', icon: <CheckIcon className="w-3 h-3" />, label: 'success' };
            case 'stage': return { bg: 'bg-brand-accent', icon: <ArrowRightIcon className="w-3 h-3 text-white" />, label: 'stage' };
            case 'warning': return { bg: 'bg-yellow-100', text: 'text-yellow-800', icon: <ExclamationIcon className="w-3 h-3 text-yellow-800" />, label: 'warning' };
            default: return { bg: 'bg-gray-200', icon: null, label: type };
        }
    };
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-7xl mx-auto space-y-6">
                 <div>
                    <h2 className="text-2xl font-bold text-brand-text-primary">Logs do App</h2>
                    <p className="text-brand-text-secondary mt-1">Monitore atividades e status do sistema</p>
                </div>
                    ...
                    <div className="bg-brand-secondary p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-semibold text-gray-500">Banco de Dados</span>
                            </div>
                            <div className="w-2 h-2 rounded-full bg-green-500"></div>
                        </div>
                        <div className="text-2xl font-normal text-gray-600 mb-1">Conectado</div>
                        <div className="text-xs text-brand-text-secondary">Última verificação: há 2 min</div>
                    </div>

                    <div className="bg-brand-secondary p-6 rounded-xl border border-gray-200 shadow-sm">
                         <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <LightningIcon className="w-6 h-6 text-gray-500" />
                                <span className="text-sm font-semibold text-gray-500">APIs Externas</span>
                            </div>
                            <div className="w-2 h-2 rounded-full bg-green-500"></div>
                        </div>
                        <div className="text-2xl font-normal text-gray-600 mb-1">3/3 ativas</div>
                        <div className="text-xs text-brand-text-secondary">Última verificação: há 1 min</div>
                    </div>
                    ...
                    <div className="bg-brand-secondary p-6 rounded-xl border border-gray-200 shadow-sm">
                         <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-semibold text-gray-500">Motores de IA</span>
                            </div>
                            <div className="w-2 h-2 rounded-full bg-yellow-400"></div>
                        </div>
                        <div className="text-2xl font-normal text-gray-600 mb-1">2/3 disponíveis</div>
                        <div className="text-xs text-brand-text-secondary">Última verificação: há 5 min</div>
                    </div>
                </div>

                <div className="bg-brand-secondary rounded-xl border border-gray-200 shadow-sm p-6">
                     <div className="flex items-center justify-between mb-8">
                        <div>
                             <h3 className="text-xl font-normal text-gray-600">Registro de Atividades</h3>
                             <p className="text-sm text-brand-text-secondary">Histórico de eventos e operações do sistema</p>
                        </div>
                        <select className="px-4 py-2 border border-brand-input-border rounded-lg bg-gray-50 text-brand-text-secondary focus:border-brand-accent outline-none text-sm">
                         ...
                         <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-gray-100">
                                    <th className="py-4 text-xs font-medium text-brand-text-secondary uppercase w-48">Data/Hora</th>
                                    <th className="py-4 text-xs font-medium text-brand-text-secondary uppercase w-40">Tipo</th>
                                    <th className="py-4 text-xs font-medium text-brand-text-secondary uppercase">Descrição</th>
                                    <th className="py-4 text-xs font-medium text-brand-text-secondary uppercase w-48 text-right">Usuário</th>
                                </tr>
                            </thead>
                                    ...
                                    return (
                                        <tr key={log.id} className="hover:bg-gray-50">
                                            <td className="py-4 text-sm text-gray-600 font-mono">
                                                {log.dateTime.split(' ').map((part, i) => (
                                                    <div key={i}>{part}</div>
                                                ))}
                                            </td>
                                            <td className="py-4">
                                                {['assignment', 'upload', 'error', 'stage'].includes(log.type) ? (
                                                     <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium text-white ${style.bg} min-w-[100px] justify-center`}>
                                                        {style.icon} {style.label}
                                                    </span>
                                                ) : log.type === 'warning' ? (
                                                     <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${style.bg} ${style.text} min-w-[100px] justify-center`}>
                                                        {style.icon} {style.label}
                                                    </span>
                                                ) : (
                                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${style.bg} ${style.text} min-w-[100px] justify-center`}>
